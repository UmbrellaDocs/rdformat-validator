...
export class RDFormatValidator {
  ...
  private fixer: Fixer;
  private options: Required<RDFormatValidatorOptions>;
   ...
   * ```
   */
  async validateString(input: string, fix: boolean = false): Promise<RDFormatValidatorResult> {
    try {
   ...
   * ```
   */
  async validateFile(filePath: string, fix: boolean = false): Promise<RDFormatValidatorResult> {
    try {
      // Parse the file
   ...
   * ```
   */
  validateObject(data: any, fix: boolean = false): RDFormatValidatorResult {
    try {
      // Validate the data
 ...
 * ```
 */
export async function isValidRDFormat(
  input: string | object,
  options: RDFormatValidatorOptions = {}
): Promise<boolean> {
...
export async function getValidationErrors(
  input: string | object,
  options: RDFormatValidatorOptions = {}
): Promise<import('./types/validation').ValidationError[]> {
  const result = await validate(input, options);
 ...
 * ```
 */
export async function validateAndFixBatch(
  inputs: (string | object)[],
  options: RDFormatValidatorOptions = {}
): Promise<RDFormatValidatorResult[]> {
  const validator = new RDFormatValidator(options);
 ...
 * ```
 */
export function createValidationSummary(results: RDFormatValidatorResult[]): {
  totalCount: number;
  validCount: number;
  invalidCount: number;
  totalErrors: number;
